    ...
    def send_request(self, search_type:int, server_id:str, trgtPlayer:str=None):
        ...
        try:     
            ...
            if search_type == 0: # Find Target Server
                results = self._find_server(base_url, server_id)
                return results
            elif search_type == 1: # Check if Player is Active
                results = self._player_active_check(base_url, server_id, trgtPlayer)
                return results
        ...
        try:
            ...
            for player in players:
                if player['attributes']['name'] == trgt_player:
                    return [f"[X] {trgt_player} is **ACTIVE**", 1] # 1 == tells method caller ACTIVE
            return [f"[ ] {trgt_player} NOT ACTIVE", 0] # 0 == tells method caller NOT ACTIVE
        except Exception as e:
